...
def display_filters(features: list, img_num: int, layers: int, cmap: str, name_layers: list, data, collapse_func=np.mean, plot: bool = True):
    """
    ...
    fig, axes = plt.subplots(1, num_layers + 1, figsize=(25, 8)) # Add 1 for the original image

    # Display the original image
    axes[0].imshow(image_data)
    axes[0].set_title("Original Image")

    for count, ax in enumerate(axes[1:], 1): # Start from 1 to leave space for the original image
...
def save_filters(features: list, img_num: int, layers: int, cmap: str, name_layers: list, data, model, collapse_func=np.mean,):
    """
    Display the original image and collapsed feature maps for each layer of interest.
